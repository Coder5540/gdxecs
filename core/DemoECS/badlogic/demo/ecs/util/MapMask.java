...
public class MapMask {
	...
	public MapMask(int height, int width, Array<TiledMapTileLayer> layers,
			String propertyKey) {
		this.height = height;
		this.width = width;
	 ...
	 * @param y
	 *            grid coordinates.
	 * @return TRUE when property found at TILE coordinates, FALSE if otherwise
	 *         or out of bounds.
	 */
	...
	public boolean atScreen(final int x, final int y) {
		return atGrid((int) (x / R.CELL_SIZE), (int) (y / R.CELL_SIZE));
	}

	public boolean atScreen(final float x, final float y) {
		return atGrid((int) ((int) x / R.CELL_SIZE),
				(int) ((int) y / R.CELL_SIZE));
	}
